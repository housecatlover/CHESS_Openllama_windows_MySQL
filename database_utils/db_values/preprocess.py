...
def _get_unique_values(db_path: str) -> Dict[str, Dict[str, List[str]]]:
    ...
    """
    table_names = [table[0] for table in execute_sql(db_path, "SHOW TABLES;", fetch="all")]
    primary_keys = []

    for table_name in table_names:
        columns = execute_sql(db_path, f"DESCRIBE `{table_name}`;", fetch="all")
        for column in columns:
            if column[3] == "PRI":  # Check if it's a primary key
    ...
    for table_name in table_names:
        ...
        columns = [col[0] for col in execute_sql(db_path, f"DESCRIBE `{table_name}`;", fetch="all") if ("char" in col[1].lower() or "text" in col[1].lower()) and col[0].lower() not in [c.lower() for c in primary_keys]]
...
def make_db_lsh(db_directory_path: str, **kwargs: Any) -> None:
    ...
    Args:
        db_directory_path (str): The path to the data path directory.
